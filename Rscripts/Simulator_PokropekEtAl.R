...
simulate_pokropek = function(n = 1500, #obs
                             g = 24, #groups
                             p = 5, #items
  ...
  Y = sapply(1:p, function(j) discretize(rnorm(n*g, tau[j] + lambda[j]*eta, eps[j])))
  ...
  for(j in 1:(h*g)){
    ...
    tau_affected[p_affected] = tau[p_affected] + sample(c(-1, 1), replace = T, k) * itembias
    lambda_affected[p_affected] = lambda[p_affected] + sample(c(-1, 1), replace = T, k) * itembias
    eps_affected = abs(1 - lambda_affected^2) # unique sd
    
    # generate data where MI doesn't hold ( STEP 5 )
    Y[g_ind == g_affected[j],] = sapply(1:p, function(j) discretize(rnorm(n, tau_affected[j] + lambda_affected[j]*eta, eps_affected[j])))
  }
  dat = as.data.frame(Y)
